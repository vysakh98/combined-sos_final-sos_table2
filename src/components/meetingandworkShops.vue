...
add(){
  ...
  this.$axios.get('http://localhost:3000/MAW/data').then(results=>{
  ...
  this.MAWId=(results.data.result[itemsLength].MAWId)+1
  }
  this.count=this.count+1
  this.editindex=this.count
  this.submitStatus='post'
  this.editable=true
  this.items.push({MAWId:this.MAWId,Personalcosts:'', Units:'',Total_No_of_Units:'',Total_Unit_value:'',Total_Total_cost:''
  ,year1_Units:'',Year1_No_of_Units:'',Year1_Unit_value:'',Year1_Total_cost:'',Detailed_Explanation:'',Actions:''})
  this.$axios({
  method: 'post',
  url:'http://localhost:3000/MAW/data/post/',
  headers: {}, 
  data: {
    MAWId:this.MAWId,
    JobTitle :'',
...
deleteitem(item){
   this.count=this.count-1
   this.MAWId=this.MAWId-1
   const index = this.items.indexOf(item)
   this.delIndex1=this.items[index].MAWId
   this.delIndex2=(this.items[index].MAWId)+1
   ...
   this.$axios.delete(`http://localhost:3000/MAW/data/delete/${this.delIndex1}`).then(result=>{
   console.log(result)
   })
   this.axios.delete(`http://localhost:3000/MAW/data/delete/${this.delIndex2}`).then(result=>{
   console.log(result)
   })
},
save(e,item,totalcost,year1cost){
  ...
  this.$axios({
  method: 'put',
  url:'http://localhost:3000/MAW/data/update/all',
  headers: {}, 
  data: {
    MAWId:this.MAWId,
...
this.$emit('workshopscost',{subtotal:this.Totalsum,year1sum:this.year1sum})
  ...
  this.EditIndex1=this.items[this.editindex].MAWId
  this.EditIndex2=(this.items[this.editindex].MAWId)+1
...
watch:{
  ...
  let Totalsum=0
  let Year1sum=0
  if(this.items.length==this.$store.getters.MAW.length){
   ...
   this.$store.getters.MAW.map(data=>{
