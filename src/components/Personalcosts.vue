...
add(){
  ...
  this.$axios.get('http://localhost:3000/PC/data').then(results=>{
  const itemsLength=(results.data.result.length-1)
  console.log(itemsLength)
  if(itemsLength==-1){
  this.PersonalCostId=0
  }
  else if(itemsLength>=0){
  this.PersonalCostId=(results.data.result[itemsLength].PersonalCostId)+1
  }
  this.count=this.count+1
  this.editindex=this.count
  this.submitStatus='post'
  this.editable=true
  this.items.push({PersonalCostId:this.PersonalCostId,Personalcosts:'', Units:'',Total_No_of_Units:'',Total_Unit_value:'',Total_Total_cost:''
  ,year1_Units:'',Year1_No_of_Units:'',Year1_Unit_value:'',Year1_Total_cost:'',Detailed_Explanation:'',Actions:''})
  this.$axios({
  method: 'post',
  url:'http://localhost:3000/PC/data/post/',
  headers: {}, 
  data: {
    PersonalCostId:this.PersonalCostId,
    JobTitle :'',
    Units:'',
    NoOfUnits:0,
    UnitValue:0,
  ...
  headers: {}, 
  data: {
    PersonalCostId:this.PersonalCostId+1,
    JobTitle:'',
    Units:'',
    NoOfUnits:0,
    UnitValue:0,
...
deleteitem(item){
   ...
   this.delIndex1=this.items[index].PersonalCostId
   this.delIndex2=(this.items[index].PersonalCostId)+1
   ...
   this.$axios.delete(`http://localhost:3000/PC/data/delete/${this.delIndex1}`).then(result=>{
   console.log(result)
   })
   this.axios.delete(`http://localhost:3000/PC/data/delete/${this.delIndex2}`).then(result=>{
   console.log(result)
   })
},
save(e,item,totalcost,year1cost){
  ...
  this.$axios({
  method: 'put',
  url:'http://localhost:3000/PC/data/update/all',
  headers: {}, 
  data: {
    PersonalCostId:this.PersonalCostId,
...
this.$emit('personalcost',{subtotal:this.Totalsum,year1sum:this.year1sum})
  ...
  this.EditIndex1=this.items[this.editindex].PersonalCostId
  this.EditIndex2=(this.items[this.editindex].PersonalCostId)+1
  ...
  headers: {}, 
  data: {
    PersonalCostId:this.EditIndex1,
    JobTitle:this.Personalcosts,
    Units:this.Units,
    NoOfUnits:parseInt(this.Total_No_of_Units),
    UnitValue:parseInt(this.Total_Unit_value),
...
this.$axios({
  ...
  headers: {}, 
  data: {
    PersonalCostId:this.EditIndex2,
    JobTitle:this.Personalcosts,
    Units:this.year1_Units,
    NoOfUnits:parseInt(this.Year1_No_of_Units),
    UnitValue:parseInt(this.Year1_Unit_value),
...
watch:{
  ...
  if(this.items.length==this.$store.getters.Pesonalcost.length){
   ...
   this.$store.getters.Pesonalcost.map(data=>{
