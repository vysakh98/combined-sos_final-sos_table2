...
add(){
  ...
  this.$axios.get('http://localhost:3000/PEM/data').then(results=>{
  ...
  this.PEMId=(results.data.result[itemsLength].PEMId)+1
  }
  this.count=this.count+1
  this.editindex=this.count
  this.submitStatus='post'
  this.editable=true
  this.items.push({PEMId:this.PEMId,Personalcosts:'', Units:'',Total_No_of_Units:'',Total_Unit_value:'',Total_Total_cost:''
  ,year1_Units:'',Year1_No_of_Units:'',Year1_Unit_value:'',Year1_Total_cost:'',Detailed_Explanation:'',Actions:''})
  this.$axios({
  method: 'post',
  url:'http://localhost:3000/PEM/data/post/',
  headers: {}, 
  data: {
    PEMId:this.PEMId,
    JobTitle :'',
...
deleteitem(item){
   this.count=this.count-1
   this.PEMId=this.PEMId-1
   const index = this.items.indexOf(item)
   this.delIndex1=this.items[index].PEMId
   this.delIndex2=(this.items[index].PEMId)+1
   ...
   this.$axios.delete(`http://localhost:3000/PEM/data/delete/${this.delIndex1}`).then(result=>{
   console.log(result)
   })
   this.axios.delete(`http://localhost:3000/PEM/data/delete/${this.delIndex2}`).then(result=>{
   console.log(result)
   })
},
save(e,item,totalcost,year1cost){
  ...
  this.$axios({
  method: 'put',
  url:'http://localhost:3000/PEM/data/update/all',
  headers: {}, 
  data: {
    PEMId:this.PEMId,
...
this.$emit('projectequipment',{subtotal:this.Totalsum,year1sum:this.year1sum})
  ...
  this.EditIndex1=this.items[this.editindex].PEMId
  this.EditIndex2=(this.items[this.editindex].PEMId)+1
...
watch:{
  ...
  let Totalsum=0
  let Year1sum=0
  if(this.items.length==this.$store.getters.PEM.length){
   ...
   this.$store.getters.PEM.map(data=>{
