...
add(){
  ...
  this.$axios.get('http://localhost:3000/Travel/data').then(results=>{
  ...
  this.TravelId=(results.data.result[itemsLength].TravelId)+1
  }
  this.count=this.count+1
  this.editindex=this.count
  this.submitStatus='post'
  this.editable=true
  this.items.push({TravelId:this.TravelId,Personalcosts:'', Units:'',Total_No_of_Units:'',Total_Unit_value:'',Total_Total_cost:''
  ,year1_Units:'',Year1_No_of_Units:'',Year1_Unit_value:'',Year1_Total_cost:'',Detailed_Explanation:'',Actions:''})
  this.$axios({
  method: 'post',
  url:'http://localhost:3000/Travel/data/post/',
  headers: {}, 
  data: {
...
deleteitem(item){
   this.count=this.count-1
   this.TravelId=this.TravelId-1
   const index = this.items.indexOf(item)
   this.delIndex1=this.items[index].TravelId
   this.delIndex2=(this.items[index].TravelId)+1
   ...
   this.$axios.delete(`http://localhost:3000/Travel/data/delete/${this.delIndex1}`).then(result=>{
   console.log(result)
   })
   this.axios.delete(`http://localhost:3000/Travel/data/delete/${this.delIndex2}`).then(result=>{
   console.log(result)
   })
},
save(e,item,totalcost,year1cost){
  ...
  this.$axios({
  method: 'put',
  url:'http://localhost:3000/Travel/data/update/all',
  headers: {}, 
  data: {
    TravelId:this.TravelId,
    ...
    DeTravelription:this.Detailed_Explanation,
    ProjectBudjetId:this.ProjectBudjetId
  }
}).then(result=>{
  console.log(result)
...
this.$emit('Travel',{subtotal:this.Totalsum,year1sum:this.year1sum})
  ...
  this.EditIndex1=this.items[this.editindex].TravelId
  this.EditIndex2=(this.items[this.editindex].TravelId)+1
...
watch:{
  ...
  let Totalsum=0
  let Year1sum=0
  if(this.items.length==this.$store.getters.Travel.length){
   ...
   this.$store.getters.Travel.map(data=>{
