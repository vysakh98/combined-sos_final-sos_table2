...
add(){
  ...
  this.$axios.get('http://localhost:3000/PS/data').then(results=>{
  const itemsLength=(results.data.result.length-1)
  console.log(itemsLength)
  if(itemsLength==-1){
  this.ProjectSupplieId=0
  }
  else if(itemsLength>=0){
  this.ProjectSupplieId=(results.data.result[itemsLength].ProjectSupplieId)+1
  }
  this.count=this.count+1
  this.editindex=this.count
  this.submitStatus='post'
  this.editable=true
  this.items.push({ProjectSupplieId:this.ProjectSupplieId,Personalcosts:'', Units:'',Total_No_of_Units:'',Total_Unit_value:'',Total_Total_cost:''
  ,year1_Units:'',Year1_No_of_Units:'',Year1_Unit_value:'',Year1_Total_cost:'',Detailed_Explanation:'',Actions:''})
  this.$axios({
  method: 'post',
  url:'http://localhost:3000/PS/data/post/',
  headers: {}, 
  data: {
    ProjectSupplieId:this.ProjectSupplieId,
    JobTitle :'',
    Units:'',
    NoOfUnits:0,
    UnitValue:0,
  ...
  headers: {}, 
  data: {
    ProjectSupplieId:this.ProjectSupplieId+1,
    JobTitle:'',
    Units:'',
    NoOfUnits:0,
    UnitValue:0,
...
deleteitem(item){
   this.count=this.count-1
   this.ProjectSupplieId=this.ProjectSupplieId-1
   const index = this.items.indexOf(item)
   this.delIndex1=this.items[index].ProjectSupplieId
   this.delIndex2=(this.items[index].ProjectSupplieId)+1
   ...
   this.$axios.delete(`http://localhost:3000/PS/data/delete/${this.delIndex1}`).then(result=>{
   console.log(result)
   })
   this.axios.delete(`http://localhost:3000/PS/data/delete/${this.delIndex2}`).then(result=>{
   console.log(result)
   })
},
save(e,item,totalcost,year1cost){
  ...
  this.$axios({
  method: 'put',
  url:'http://localhost:3000/PS/data/update/all',
  headers: {}, 
  data: {
    ProjectSupplieId:this.ProjectSupplieId,
...
.catch(err=>{
  ...
  this.EditIndex1=this.items[this.editindex].ProjectSupplieId
  this.EditIndex2=(this.items[this.editindex].ProjectSupplieId)+1
  ...
  headers: {}, 
  data: {
    ProjectSupplieId:this.EditIndex1,
    JobTitle:this.Personalcosts,
    Units:this.Units,
    NoOfUnits:parseInt(this.Total_No_of_Units),
    UnitValue:parseInt(this.Total_Unit_value),
...
this.$axios({
  ...
  headers: {}, 
  data: {
    ProjectSupplieId:this.EditIndex2,
    JobTitle:this.Personalcosts,
    Units:this.year1_Units,
    NoOfUnits:parseInt(this.Year1_No_of_Units),
    UnitValue:parseInt(this.Year1_Unit_value),
...
watch:{
  ...
  let Totalsum=0
  let Year1sum=0
  if(this.items.length==this.$store.getters.ProjectSupplie.length){
   ...
   this.$store.getters.ProjectSupplie.map(data=>{
